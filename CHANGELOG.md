...
### Added
- **Firmware / BLE** — `GET_STATUS` now includes a `BAT_LOG=<seconds>` field when battery telemetry logging is active (see `START_BAT_LOG`). The field is omitted entirely when logging is off, preserving the existing response format for clients that don't expect it.
- **Firmware / BLE / m5ctl** — Battery telemetry logging. Four new BLE commands: `START_BAT_LOG [<seconds>]` enables periodic sampling (default every 300 s, range 10–86400 s, clamped) and appends a `BOOT` marker immediately; `STOP_BAT_LOG` disables logging while retaining data; `GET_BAT_LOG [<page>]` retrieves up to 16 entries per call (paged) in oldest-first order; `CLEAR_BAT_LOG` erases all entries. Events logged: `BOOT` (device power-on or `START_BAT_LOG`), `SAMPLE` (periodic), `BLE_CON` / `BLE_DIS` (BLE connect / disconnect), `SCR_CHG` (screen navigation). Each entry carries a UTC timestamp, battery voltage (mV), battery level (0–100 or 255=unknown), and active screen index. Data is stored in a 128-entry ring buffer in NVS namespace `"batlog"` and survives hardware power-off. The active flag and sample interval also persist across reboots, so logging resumes automatically after power-cycling the device. Logging is opt-in: NVS is never written during normal operation unless `START_BAT_LOG` has been sent. Four new `m5ctl` subcommands: `bat-log-start [--interval SECS]`, `bat-log-stop`, `bat-log` (paged retrieval with ISO8601 table output), `bat-log-clear`. The BATTERY screen gains a sparkline of historical charge levels when at least 2 SAMPLE entries exist. Suggested by [@senshu-hiro2](https://github.com/senshu-hiro2).
...
- **m5ctl** — `m5ctl -h` / `--help` now shows the version number in the help banner (`m5ctl <VERSION> — Control the M5StickC NexStar clinometer over BLE.`), so the version is visible without running the separate `m5ctl version` subcommand.
- **`tests/3d_model.py`** — `-h` / `--help` now shows a version number in the help banner (`3d_model <VERSION> — 3D orientation visualizer for M5Stack BLE clinometer`), matching `m5ctl`'s banner. The pre-commit hook keeps its `_VERSION` constant in sync with firmware's `FW_VERSION` using the same gating as `m5ctl`: stamped only in commits that already touch `3d_model.py` or when the MAJOR.MINOR version changes.
- **m5ctl `bat-log`** — `--csv` flag prints the battery log as CSV instead of the fixed-width table: header row `unix_epoch,timestamp_utc,type,battery_pct,voltage_mv,screen`, one row per entry. Voltage is reported in raw millivolts (integer, no unit suffix) rather than volts, avoiding decimal-point/comma locale ambiguity when importing into a spreadsheet. Both a Unix epoch and an ISO8601 UTC timestamp column are included — the former for sorting/plotting without date parsing, the latter for spreadsheets and `pandas.to_datetime`. Missing values (`utcSec=0`, `batLevel=255`, `batMv=0`) are emitted as blank fields rather than `?`/text placeholders, so `pandas.read_csv`/spreadsheets parse them as `NaN`/empty rather than breaking numeric columns. Malformed log entries are skipped with a warning on stderr instead of corrupting the CSV structure.

### Changed
- **Firmware / Display** — BATTERY screen layout when logging is active: the green fill-bar is now hidden (it duplicates what the sparkline already conveys) and the freed vertical space is redistributed across four rows computed from actual font metrics rather than proportional screen fractions — title (Font4 height + 2 px gap), voltage/% centred in the remaining slot above the graph, sparkline graph filling from that slot down to the footer, and the `[B]` nav hint (Font2 height + 2 px from the bottom). The graph gains approximately 4× the pixel height it previously had (≈68 px vs 16 px), making the discharge history clearly legible. When logging is *not* active the screen is unchanged. Suggested by [@senshu-hiro2](https://github.com/senshu-hiro2).

### Fixed
- **Firmware / Display** — Dim colours encoded as `0x2104` (RGB565 R=G=B=4/31) in the BATTERY screen sparkline grid lines, the clinometer angle screen's 10-segment battery bar, and the sysinfo stack-usage bars were invisible on 8-bit colour sprites: the RGB332 palette cannot represent a colour this dark and rounds it to black, making the elements indistinguishable from the background. Replaced with `TFT_DARKGREY` throughout. Reported and patch proposed by [@senshu-hiro2](https://github.com/senshu-hiro2).
- **Firmware / Display** — The dark-green colour (`0x0340`, G=13/63) used in the clinometer angle screen's 10-segment battery bar shared the same 8-bit invisibility problem: G=13/63 maps to G=1 in RGB332, which renders as black on 8-bit colour sprites. Replaced with `TFT_DARKGREEN` (G=31/63, maps to G=3 in RGB332, clearly visible). The dark-red (`0x6000`) is simultaneously replaced with `TFT_MAROON` for named-constant consistency; the dark-amber (`0x7A40`, R=3 G=2 in RGB332) is retained as a raw literal because it is already clearly visible on 8-bit sprites and no standard named constant matches its hue. Reported and patch proposed by [@senshu-hiro2](https://github.com/senshu-hiro2).
- **Firmware / Display** — Stopping battery logging (`STOP_BAT_LOG`) left the BATTERY screen in sparkline layout even though the data was no longer being updated, giving no indication of how stale the graph was. The sparkline is now gated on `BatLog::isActive()` in addition to the entry count, so the screen reverts to the fill-bar as soon as logging is stopped.
